...
export default class Car {
  ...
  setModels() {
    ...
    if (this.config.cyberTruck) {
      ...
      this.models.backLightsBrake =
        this.resources.items.carCyberTruckBackLightsBrake;
      this.models.backLightsReverse =
        this.resources.items.carCyberTruckBackLightsReverse;
    ...
    // Default
      ...
      this.models.backLightsBrake =
        this.resources.items.carDefaultBackLightsBrake;
      this.models.backLightsReverse =
        this.resources.items.carDefaultBackLightsReverse;
  ...
  setMovement() {
    ...
    this.time.on("tick", () => {
      ...
      this.movement.acceleration = movementSpeed
        .clone()
        .sub(this.movement.speed);
      ...
      this.sounds.engine.acceleration = this.controls.actions.up
  ...
  setChassis() {
    ...
    this.time.on("tick", () => {
      ...
      if (!this.transformControls.enabled) {
        ...
        this.chassis.object.quaternion.copy(
  ...
  setAntena() {
    ...
    this.antena.object = this.objects.getConvertedMesh(
      this.models.antena.scene.children
    );
    ...
    this.time.on("tick", () => {
      ...
      const accelerationX = Math.min(
        Math.max(this.movement.acceleration.x, -max),
        max
      );
      const accelerationY = Math.min(
        Math.max(this.movement.acceleration.y, -max),
        max
      );
      ...
      const pullBack = position
        .negate()
        .multiplyScalar(position.length() * this.antena.pullBackStrength);
      ...
      this.antena.localPosition.rotateAround(
        new THREE.Vector2(),
        -this.chassis.object.rotation.z
      );
  ...
  setBackLights() {
    ...
    this.backLightsBrake.object = this.objects.getConvertedMesh(
      this.models.backLightsBrake.scene.children
    );
    ...
    this.backLightsReverse.object = this.objects.getConvertedMesh(
      this.models.backLightsReverse.scene.children
    );
    ...
    // Time tick
    this.time.on("tick", () => {
      this.backLightsBrake.material.opacity = this.physics.controls.actions
        .brake
        ? 1
        : 0.5;
      this.backLightsReverse.material.opacity = this.physics.controls.actions
        .down
        ? 1
        : 0.5;
    });
  ...
  setWheels() {
    this.wheels = {};
    this.wheels.object = this.objects.getConvertedMesh(
      this.models.wheel.scene.children
    );
    this.wheels.items = [];
  ...
  setShootingBall() {
    ...
    window.addEventListener("keydown", (_event) => {
      if (_event.key === "b") {
        ...
        bowlingBall.collision.body.applyImpulse(
          direction,
          bowlingBall.collision.body.position
        );
      }
    });
  }
